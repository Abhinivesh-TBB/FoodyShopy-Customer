...
class MockApiInterceptor extends Interceptor {
  ...
  void onRequest(
    ...
    if (path.endsWith('/customer/restaurants') ||
    ...
    if (path.endsWith('/customer/offers/validate') ||
        path == '/customer/offers/validate') {
      ...
      final discountVal = _calculateDiscount(code, cartTotal);
      ...
      return handler.resolve(
        Response(
          requestOptions: options,
          statusCode: 200,
          data: {'discount_amount': discountVal, 'total_amount': finalTotal},
        ),
      );
    }
    ...
    if ((path.endsWith('/customer/orders') || path == '/customer/orders') &&
      ...
      for (final itemReq in itemsReq) {
        ...
        cartTotal += menuItem.price * qty;
        orderItems.add(
          OrderItem(name: menuItem.name, quantity: qty, price: menuItem.price),
        );
      }
      ...
      final double grandTotal =
          (subtotal + deliveryFee + platformFee + gst - discountVal).clamp(
            0.0,
            double.infinity,
          );
      ...
      final formattedDate =
          '${now.day.toString().padLeft(2, '0')} ${months[now.month - 1]} ${now.year}, ${hour.toString().padLeft(2, '0')}:$min $period';
    ...
    if ((path.endsWith('/customer/orders') || path == '/customer/orders') &&
        options.method == 'GET') {
      final mappedOrders = _orders.map(_orderToJson).toList();

      return handler.resolve(
        Response(requestOptions: options, statusCode: 200, data: mappedOrders),
      );
    }

    // 8. GET /customer/orders/:id/delivery-otp
    final otpMatch = RegExp(
      r'/customer/orders/([^/]+)/delivery-otp$',
    ).firstMatch(path);
    if (otpMatch != null) {
      return handler.resolve(
        Response(
  ...
  double _calculateDiscount(String? code, double subtotal) {
    ...
    switch (code.trim().toUpperCase()) {
      case 'WELCOME50':
        return (subtotal * 0.5).clamp(0.0, 100.0);
