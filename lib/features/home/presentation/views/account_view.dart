...
class AccountView extends ConsumerWidget {
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () =>
                          _showEditProfileSheet(context, ref, profileState),
                    ),
                  ],
                ),
              ),
                        ...
                        child: Row(
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () =>
                                  _showAddressSheet(context, ref, address),
                            ),
                            IconButton(
                              icon: const Icon(
  ...
  // Visual card builder for Swiggy aesthetics
  ...
  void _showAddressSheet(
    BuildContext context,
    WidgetRef ref,
    dynamic editingAddress,
  ) {
    final formKey = GlobalKey<FormState>();
    final labelController = TextEditingController(
      text: editingAddress?.label ?? '',
    );
    final addressController = TextEditingController(
      text: editingAddress?.addressLine ?? '',
    );
              ...
              PrimaryButton(
                ...
                onPressed: () {
                  if (formKey.currentState!.validate()) {
                    ...
                    // If label changed during edit, delete old label first
                    if (editingAddress != null &&
                        editingAddress.label != labelController.text.trim()) {
                      notifier.deleteAddress(editingAddress.label);
                    }

                    notifier.saveAddress(
                      labelController.text.trim(),
                      addressController.text.trim(),
                      12.9716, // Indiranagar default Lat
                      77.6408, // Indiranagar default Lng
                    );
                    Navigator.pop(context);
                    AppSnackbar.showSuccess(
                      context,
  ...
  void _showAddPaymentSheet(BuildContext context, WidgetRef ref) {
    ...
    // Track selected type inside bottom sheet state
                  ...
                  onPressed: () {
                    if (formKey.currentState!.validate()) {
                      ...
                      if (selectedType == PaymentType.card) {
                        // Mask the card number to last 4 digits
